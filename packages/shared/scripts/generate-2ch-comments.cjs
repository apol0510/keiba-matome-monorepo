...
7. **【超重要】情報の正確性を最優先**：
   - 馬の脚質（逃げ・先行・差し・追い込み）は、記事要約に明記されていない限り**絶対に言及しないこと**
   - 騎手名は、記事要約に明記されていない限り**絶対に言及しないこと**
   - 馬番と馬名のみ使用し、それ以外の詳細情報は**推測や創作を一切しないこと**
   - 例: 「12番は追い込み馬だから...」→ NG（脚質情報がない場合）
   - 例: 「森泰斗騎手が乗るから...」→ NG（騎手情報がない場合）
   - 例: 「12番強そうだな」→ OK（馬番のみで一般的なコメント）`;
