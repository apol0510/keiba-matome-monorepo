...
1. ✅ **中央重賞予想スクレイピング実装（Yahoo!スポーツ版）**
   - 問題: netkeiba予想コラムのHTML構造変更でスクレイピング失敗（0件取得）
   - 解決策: Yahoo!スポーツ競馬ページをスクレイピング元に変更
   - 実装内容:
     - scrape-chuou-yosou.cjs を完全リライト
     - `.sn-timeLine__item` セレクタで記事取得
     - G1/G2/G3/金杯/重賞/予想キーワードでフィルタリング
     - SourceSite: 'その他'（Airtable既存選択肢）使用
   - 結果: 1月4日の中山金杯GIII・京都金杯GIIIを正常に取得
   - コミット: b5707f0

2. ✅ **URL構造の中央・南関分離実装**
   - 背景: SEO向上とユーザビリティ改善のため、カテゴリ別にURLを分離
   - 実装内容:
     - `/chuou/[slug].astro` 作成（中央重賞専用）
     - `/nankan/[slug].astro` 作成（南関重賞・南関メイン専用）
     - カテゴリフィルタリング追加（誤った記事は404リダイレクト）
     - トップページにタブ機能実装（中央/南関の切り替え）
     - `getArticlePath()` 関数で動的URL生成
     - post-to-x.cjs のURL生成ロジックをカテゴリ対応
     - プリフェッチ機能の更新（`/chuou/`, `/nankan/` 対応）
   - SEO効果:
     - URL構造が明確化（/chuou/ は中央、/nankan/ は南関）
     - カテゴリ別sitemap生成が容易に
     - ユーザーの回遊性向上
   - コミット: cdcefe3

3. ✅ **スレッドタイトルに日付追加（中央・南関）**
   - 背景: いつのレースか一目でわかるように
   - 実装内容:
     - scrape-chuou-yosou.cjs: タイトル末尾に【月/日】形式追加
     - scrape-nankan-daily.cjs: 同様に日付追加
     - 例: 「【中山 GIII】中山金杯 予想スレ【1/4】」
   - 効果:
     - 視認性向上
     - 2ch風のフォーマット統一
   - コミット: 445efa4, ea067a6

4. ✅ **全画面でテーブル形式に統一**
   - 背景: ユーザーからの要望（中央タブのデザインに統一）
   - 実装内容:
     - モバイル版のカードレイアウトを削除
     - 全画面サイズでテーブルレイアウト採用
     - ヘッダー: スレッドタイトル、レス数、勢い、最終更新
     - レスポンシブ対応（モバイルは11px、デスクトップは13px）
   - 効果:
     - UI統一による視認性向上
     - 情報の一覧性が向上
   - コミット: 86c8d74
...
5. ✅ **南関重賞タブをデフォルト表示に変更**
   - 背景: 南関は平日開催が多く、更新頻度が高い
   - 実装内容:
     - タブボタンのアクティブ状態を南関に変更
     - コンテンツテーブルの初期表示を南関に変更
   - 効果:
     - ユーザーが最初に見るのは更新頻度の高い南関予想
     - ユーザーの本業（南関競馬予想）とマッチ
   - コミット: 852acca
