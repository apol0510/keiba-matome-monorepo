...
### 2026-01-06: サイトの役割分担明確化（地方競馬記事フィルタリング）

24. ✅ **地方競馬記事の自動フィルタリング実装**
    - **問題**: keiba-matome.jp（中央競馬）で地方競馬の記事が混入
    - **原因**: netkeibaトップページには中央・地方の両方が混在

    - **実装内容**:
      - `isChihouKeiba()` 関数を全スクレイピングスクリプトに追加
      - **判定キーワード**（27種類）:
      ...
      - **修正ファイル**:
        - `scripts/scrape-netkeiba-news.cjs`
        - `scripts/scrape-yahoo-news.cjs`

    - **動作**:
      - 地方競馬記事を検出 → スキップ（chihou.keiba-matome.jpで扱います）
      - 中央競馬記事のみ保存
